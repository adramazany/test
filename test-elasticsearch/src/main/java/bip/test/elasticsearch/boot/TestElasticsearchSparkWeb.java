...
/**
 * Created by ramezani on 6/11/2018.
 */
public class TestElasticsearchSparkWeb {
/*
    ...
    public static void main(String[] args) throws Exception {
        ...
        Spark.get("/", (request, response) -> {
            SearchResponse searchResponse = client.prepareSearch("music1")
                    .setTypes("songs").execute().actionGet();
        ...
        Spark.get(
                "/search",
                (request, response) -> {
                    SearchRequestBuilder srb = client.prepareSearch("music1")
                            .setTypes("songs");

                    String lyricParam = request.queryParams("query");
                    QueryBuilder lyricQuery = null;
                    if (lyricParam != null && lyricParam.trim().length() > 0) {
                        lyricQuery = QueryBuilders.matchQuery("songs",
                                lyricParam);
                        // srb.setQuery(qb).addHighlightedField("songs", 0, 0);
                    }
                    ...
                    if (lyricQuery != null && artistQuery == null) {
                        srb.setQuery(lyricQuery).addHighlightedField("songs",
                                0, 0);
        ...
        Spark.post("/save", (request, response) -> {
            ...
            json.append("\"songs\":\""+request.raw().getParameter("songs")+"\"}");

            IndexRequest indexRequest = new IndexRequest("music1", "songs",
